...
{
    public class BDExtendingRail : PartModule
    {

        [KSPField(isPersistant = true)] public float railLength = 1;

        [KSPField(isPersistant = true)] public float extensionDistance = 1;

        [KSPField(isPersistant = true)] public float extensionTime = 0.2f;

        [KSPField(isPersistant = true)] public bool railExtended = false;
        ...
        [KSPEvent(guiActive = true, guiActiveEditor = true, guiName = "Extend Rail Toggle", active = true)]//Extend
        ...
        public void ExtendRail()
        {
            ...
            while (sym.MoveNext())
            {
                ...
                sym.Current.FindModuleImplementing<BDExtendingRail>().UpdateHeight(railHeight);
            }
            sym.Dispose();
        }

        public void RetractRail()
        {
            railHeight = minRailHeight;
            railHeightTransform.localPosition = new Vector3(0, railHeight, 0);
        ...
        public void IncreaseLength()
        {
            ...
            {
                if (sym.Current == null) continue;
                sym.Current.FindModuleImplementing<BDExtendingRail>().UpdateLength(railLength);
            }
            sym.Dispose();
        }
