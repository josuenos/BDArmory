...
namespace BDArmory.Modules
{
	public class ModuleWeapon : EngageableWeapon, IBDWeapon
	{
		...
		[KSPField]
		public float maxTargetingRange = 5000; //max range for raycasting and sighting
		...
		public void ToggleRipple()
		{
			...
			while (craftPart.MoveNext())
			{
				...
				//if (craftPart.Current.name != part.name) continue; // would this potentially result in a group of two different types of weapons only toggling ripple on one of them?
		...
		public void Start()
		{
			...
			if (HighLogic.LoadedSceneIsFlight)
			{
				...
				//if ((eWeaponType != WeaponTypes.Rocket) || (eWeaponType == WeaponTypes.Rocket && externalAmmo))
				//{// start ammo tracking for non rocketpods
			...
			//setup animations
			//if (!string.IsNullOrEmpty(deployAnimName) && eWeaponType == WeaponTypes.Rocket) // hack to grab the hasAnim bool for legacy RLs converted via MM
			//{
			//	hasDeployAnim = true;
			//}
			if (hasDeployAnim)
		...
		//new regions sorting/organizing gun, laser, and rocket specific code into their own sections for ease of maintainence. Everything else is shared code
		#region Gun Fire 
